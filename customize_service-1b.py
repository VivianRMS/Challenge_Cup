            ...
            try:
                if frame is not None:
                    ...
                    if len(faces) > 0:
                        ...
                        pre_end = time.time()
                        pre_time += int(np.round((pre_end - pre_start) * 1000))
                        ...
                        test_end = time.time()
                        test_time += int(np.round((test_end - test_start) * 1000))
#                         print(mar)
                        mouth_end = time.time()
...
#                         print(len(f.lms), f.euler)
                        ...
                        test_end = time.time()
                        test_time += int(np.round((test_end - test_start) * 1000))
                        head_time += int(np.round((test_end - test_start) * 1000))
        ...
        print(f"Average getFrameTime is: {ave_getFrame_time}")

        ave_pre_time = pre_time/frame_num
        print(f"Average pre_time is: {ave_pre_time}")

        ave_test_time = test_time/frame_num
        print(f"Average test_time is: {ave_test_time}")

        ave_head_time = head_time/frame_num
        print(f"Average head time is: {ave_head_time}")

        ave_eye_time = eye_time/frame_num
        print(f"Average eye_time is: {ave_eye_time}")

        ave_mouth_time = mouth_time/frame_num
        print(f"Average mouth time is: {ave_mouth_time}")
        return result
