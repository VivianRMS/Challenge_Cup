...
class fatigue_driving_detection(PTServingBaseService):
    def __init__(self, model_name, model_path):
        ...
        self.frame = 0
        self.begin_get_frame_flag = True
            ...
            while self.input_reader.is_open():
                if self.begin_get_frame_flag:
                    if not self.input_reader.is_open() or self.need_reinit == 1:
                        ...
                        if self.input_reader.name != self.input_reader.name:
                            print(f"Failed to reinitialize camera and got {self.input_reader.name} instead of {self.input_reader.name}.")
                            # sys.exit(1)
                        self.need_reinit = 2
                        time.sleep(0.02)
                        continue
                ...
                ret, self.frame = self.input_reader.read()

                self.need_reinit = 0

                self.begin_get_frame_flag = False

                if self.frame is not None:
                        # 剪裁主驾驶位
                        frame = self.frame[:, 600:1920, :]
    ...
    def _inference(self, data):
        ...
        getFrameThread = threading.Thread(target=self.getFrame, daemon=True).start()

        while True:
            if not self.begin_get_frame_flag:
                try:
                    if self.frame is not None:
                        # 检测驾驶员是否张嘴、闭眼、转头
                        faces = self.tracker.predict(self.frame)
